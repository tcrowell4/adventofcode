...
def split_adapter(adapters):
    # split the adapters each time the jolt=3
    # return list up to and including jolt3
    pass
...
def process_adapters(adapters):
    ...
    while idx < len(adapters):
        ...
        while next_idx < len(adapters):
            ...
            # if the joltage is 3 no longer a need to investigate that path further
            if current_adapter == 3:
                next_list = adapters[:next_idx] + adapters[next_idx:]
                # next_list = adapters[:]
                prefix = prefix + "...."
                print("{} Next iteration Jolt=3: {}".format(prefix, next_list))
            ...
            if jolt_diff <= 3:
                ...
                if jolt_diff == 1:
                    next_list = adapters[:next_idx] + adapters[next_idx:]
                    # next_list = adapters[:]
                    print("{} Pop {}".format(prefix, next_list[next_idx]))
                    next_list.pop(next_idx)
                    prefix = prefix + "...."
                    print("{} Next iteration: {}".format(prefix, next_list))
                    ret = process_adapters(next_list)
                    if ret:
                        print(prefix, "*****", ret)
                        valid_combos.append(ret)
                ...
                print(prefix, "invalid joltage {}".format(jolt_diff))
                return 0
            next_idx += 1
...
def main():
    ...
    new_adapters = []
    last_adapter = 0
    for i, adapter in enumerate(slines):
        new_adapters.append(adapter)
        """
        print(">> new_adapters list", new_adapters)
        print(
            "adapter {} last_adapter {} diff {}".format(
                adapter, last_adapter, adapter - last_adapter
            )
        )
        """
        if adapter - last_adapter == 3:
            print(">>>>> new_adapters list", new_adapters)
            ret = process_adapters(new_adapters)
            print(ret)
            new_adapters = []
            #  print("len(adapters) {}".format(len(adapters)))
